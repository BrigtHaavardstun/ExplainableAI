    ...
    for x in parts:
        ...
        for i,e in enumerate(x):
            ...
            if i +1 < len(x) and x[i +1] == "'":
                diff_var.append(f"(not {e})")
            else:
                diff_var.append(e)
        curr_part = " and ".join(diff_var)
        all_parts.append(f"({curr_part})")
    
    final = " or ".join(all_parts)
    return final
        ...
        for b in [False, True]:
            for c in [False, True]:
                for d in [False, True]:
                    ...
                    print(boolExpr.evaluate(A=a, B=b, C=c, D=d))
    """
